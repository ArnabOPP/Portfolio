...
const highlightContent = {
    education: {
        ...
        description: "Every line of code starts with understanding. My academic journey at IEM Saltlake, pursuing B.Tech in CSE with AI & ML specialization, shapes how I approach complex problems with systematic thinking."
    },
    journey: {
...
function ExperienceTabSlider({ isLowPowerMode }: { isLowPowerMode: boolean }) {
    ...
    const tabs: TabItem[] = [
        { id: 'education', label: 'Education', description: 'Building strong foundations through academic excellence at IEM Saltlake and Howrah Zilla School.' },
        ...
        >
            <div className="p-6 md:p-8 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                <div className="flex gap-4 md:gap-6 items-start">
                    ...
                    <div className="w-14 h-14 md:w-16 md:h-16 bg-white dark:bg-black rounded-2xl flex items-center justify-center shrink-0 border border-neutral-100 dark:border-neutral-800 p-2 shadow-sm overflow-hidden">
                        {exp.logo ? (
...
function LogoWithFallback({ src, alt }: { src: string; alt: string }) {
    ...
    const initials = alt.split(/\s+/).slice(0, 2).map(w => w[0]).join('').toUpperCase();
    if (failed) {
        return (
            <div className="w-full h-full flex items-center justify-center rounded-lg bg-gradient-to-br from-neutral-700 to-neutral-600">
            ...
            <div className="flex items-center gap-4 relative z-10 w-full">
                ...
                {logo && (
                    <div className="shrink-0 w-12 h-12 md:w-16 md:h-16 rounded-lg bg-white dark:bg-neutral-800 border border-neutral-100 dark:border-neutral-700 overflow-hidden p-2">
                        <LogoWithFallback src={logo} alt={title || 'Logo'} />
                    </div>
                )}
            </div>
...
function TimelineGallery({ images, id, title, externalLink, logo }: { images: string[]; id: string; title?: string; externalLink?: string; logo?: string }) {
    ...
    return (
        <div className="space-y-4">
            ...
            {/* Lightbox Overlay - Portal to avoid removeChild conflicts */}
