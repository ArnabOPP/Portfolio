...
function CertificateImageGallery({ achievement }: { achievement: Achievement }) {
    ...
    if (images.length === 0) {
        return (
            <div className="w-full flex flex-col items-center justify-center gap-6 py-12 px-8 rounded-xl bg-background/20 border border-border/10">
                <div className="w-24 h-24 flex items-center justify-center rounded-3xl bg-gradient-to-br from-zinc-700 via-zinc-600 to-zinc-500 shadow-xl">
                    <Award className="w-12 h-12 text-white/80" />
                </div>
                <div className="text-center space-y-2">
                    <p className="text-sm font-bold text-foreground/70 tracking-tight">{achievement.issuer}</p>
                    <p className="text-[11px] text-muted-foreground/60 font-mono uppercase tracking-widest">Certificate image not available</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="w-full flex flex-col gap-3">
            <motion.div
                key={achievement.id + "-image-" + activeIdx}
            ...
            >
                {src.endsWith('.pdf') ? (
                    <div className="w-full aspect-[1.4] relative rounded-lg overflow-hidden border border-border/20 shadow-xl bg-white">
                        <iframe src={`${src}#toolbar=0&view=FitH`} className="w-full h-full border-0" title={achievement.title} />
                    </div>
                ) : (
                    <img
                        src={src}
                        alt={`${achievement.title} — ${activeIdx + 1}`}
                ...
                {images.length > 1 && (
                    <>
                        <button
                            ...
                            className="absolute left-4 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/60 backdrop-blur-md border border-white/10 flex items-center justify-center text-white hover:bg-black/80 transition-colors z-10"
                        >
                            <ChevronLeft className="w-4 h-4" />
                        </button>
                        <button
                            onClick={() => setActiveIdx(i => (i + 1) % images.length)}
                            className="absolute right-4 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/60 backdrop-blur-md border border-white/10 flex items-center justify-center text-white hover:bg-black/80 transition-colors z-10"
                        >
                            ...
                            {images.map((_, i) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        i === activeIdx ? "w-5 bg-white" : "w-1.5 bg-white/40 hover:bg-white/60"
                                    )}
                                />
                            ))}
                        </div>
                ...
                <div className="absolute top-4 right-4 px-2 py-1 rounded-md bg-black/50 backdrop-blur-md border border-white/10 z-10">
                    <span className="text-[9px] font-mono text-white/70 uppercase tracking-widest">{activeIdx + 1} / {images.length}</span>
                </div>
            </motion.div>
                    ...
                    {images.map((img, i) => (
                        <button
                            ...
                            className={cn(
                                ...
                                i === activeIdx ? "border-foreground/70 opacity-100" : "border-border/20 opacity-50 hover:opacity-80"
                            )}
                        >
            ...
            >
                ...
                <div className="flex flex-col lg:flex-row items-stretch p-6 md:p-8 lg:p-10 gap-10 overflow-auto no-scrollbar max-h-[85vh]">
                    ...
                    <div className="lg:w-[55%] flex flex-col gap-4">
                        <CertificateImageGallery achievement={achievement} />
                    </div>
                                ...
                                <div className="flex flex-wrap items-center gap-3">
                                    ...
                                    {achievement.credentialUrl && (
                                        ...
                                        >
                                            ...
                                            <span>{achievement.credentialUrl.includes('linkedin.com') ? 'View on LinkedIn' : 'Verify Credential'}</span>
                                            <ExternalLink className="w-3.5 h-3.5 group-hover:rotate-12 transition-transform" />
                                        </a>
                                    )}
                                    {achievement.credentialUrl && !achievement.credentialUrl.includes('linkedin.com') && (
                                        <a
                                            href="https://www.linkedin.com/in/arnab-chatterjee-prsnl/details/certifications/"
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="group relative inline-flex items-center justify-center gap-2.5 px-4 py-2 bg-[#0A66C2] text-white font-black rounded-xl text-[9px] uppercase tracking-[0.2em] transition-all hover:-translate-y-0.5 active:scale-95 shadow-lg overflow-hidden"
                                        >
