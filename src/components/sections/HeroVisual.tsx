...
export function HeroVisual({ isExiting }: { isExiting?: boolean }) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="relative h-screen w-full flex flex-col bg-background text-foreground overflow-hidden selection:bg-primary/20"
    >
      {/* Background Pattern */}
      ...
      {/* Person photo — masked to remove white background, sits in front of text */}
      <div className="absolute inset-0 z-20 pointer-events-none overflow-hidden">
        <div className="absolute inset-0 flex items-end justify-center">
          <motion.img
            ...
            className="h-[100vh] w-auto object-contain object-top select-none"
          />
        </div>
      </div>

      <main className="relative flex-1 flex flex-col justify-center pt-[18vh] pb-[4vh]">
        <div className="flex relative gap-4 px-6 md:items-center w-full flex-col justify-center">
          ...
          <div className="md:flex gap-8 items-center relative">
            <div className="relative">
              ...
              {/* Outline overlay — transparent fill, thin white stroke, above photo */}
              <motion.h1
                initial={{ opacity: 0, y: 30 }}
                animate={isExiting ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
                transition={{ duration: 1.2, delay: 0.1, ease: [0.16, 1, 0.3, 1] }}
                className="absolute inset-0 text-[clamp(3rem,11vw,13rem)] md:flex items-center font-black leading-[0.85] tracking-tighter will-change-transform px-4 z-30 pointer-events-none select-none"
                style={{ color: 'transparent', WebkitTextStroke: '1px white' }}
              >
                <span>SOFT</span>
                <div className="hidden lg:block mx-[0.05em] opacity-0">
                  <Zap className="w-[0.8em] h-[0.8em]" strokeWidth={1.5} />
                </div>
                <div className="block lg:hidden mx-[0.02em] opacity-0">
                  <Zap className="w-[0.8em] h-[0.8em]" strokeWidth={2} />
                </div>
                <span>WARE</span>
              </motion.h1>
            </div>
          ...
          <div className="md:flex gap-8 items-center relative z-30">
            <motion.h1
              initial={{ opacity: 0, y: 30 }}
              animate={isExiting ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
        ...
        {/* Separator Section */}
        <div className="mx-auto max-w-[105rem] w-full px-8 md:px-20 mt-auto pb-4 md:pb-6">
