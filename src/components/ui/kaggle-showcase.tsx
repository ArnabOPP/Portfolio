...
const KAGGLE_USER = "arnabchatterjee05";
    ...
    return (
      <section id='kaggle-stats' className='w-full max-w-[1700px] mx-auto px-6 pt-10 pb-24 md:pt-14 md:pb-32'>
        <motion.div
          ...
          viewport={{ once: true }}
          className="relative bg-white dark:bg-[#0A0A0A] border border-black/5 dark:border-white/10 rounded-[3rem] shadow-2xl overflow-hidden p-8 md:p-14"
        >
          {/* Animated background pulse */}
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full bg-[#20beff]/5 animate-ping" style={{ animationDuration: '3s' }} />
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] h-[300px] rounded-full bg-[#20beff]/5 animate-ping" style={{ animationDuration: '2s', animationDelay: '0.5s' }} />
          </div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
            <div className="space-y-6 max-w-xl">
              ...
              <h2 className="text-4xl md:text-5xl font-bold tracking-tight leading-[0.9em] text-black dark:text-white">
                Kaggle profile <br />
                <span className="text-[#20beff]">Coming Soon.</span>
              </h2>
              <p className="text-black/50 dark:text-white/40 text-lg leading-relaxed">
                Just getting started on Kaggle. Notebooks, datasets and competition entries will appear here automatically once published.
              </p>
              <a
                href={`https://www.kaggle.com/${KAGGLE_USER}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 bg-[#20beff] text-black font-bold rounded-full text-sm hover:bg-[#20beff]/80 transition-colors"
              >
            ...
            <div className="flex flex-col items-center gap-4 opacity-30">
              <div className="grid grid-cols-2 gap-3">
                {['Notebooks', 'Datasets', 'Competitions', 'Models'].map(label => (
                  <div key={label} className="w-32 h-20 rounded-2xl border border-[#20beff]/20 flex flex-col items-center justify-center gap-1">
                    <span className="text-2xl font-black text-[#20beff]">0</span>
                    <span className="text-[10px] uppercase tracking-widest text-black dark:text-white font-bold">{label}</span>
                  </div>
                ))}
              </div>
              <span className="text-[10px] uppercase tracking-widest text-black/30 dark:text-white/30 font-bold">Updates automatically</span>
            </div>
          </div>
        </motion.div>
      </section>
