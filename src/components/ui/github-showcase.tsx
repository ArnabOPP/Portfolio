...
const GITHUB_USER = "ArnabOPP";

const PINNED_REPOS = [
  {
    name: "Lightswind-UI-Library",
    desc: "Production-ready animated React component library. Open-source UI kit with 160+ components, Smart CLI, and CSS variable theming built with Tailwind CSS and Framer Motion.",
    stars: 0,
    forks: 0,
    lang: "TypeScript",
    url: "https://github.com/ArnabOPP/Lightswind-UI-Library"
  },
  {
    name: "ThermalSense-AI",
    desc: "AI-powered thermal anomaly detection system built for ISRO Antariksh Hackathon PS1. Integrates multi-sensor data streaming with localized AI control loops for real-time space hardware optimization.",
    stars: 0,
    forks: 0,
    lang: "Python",
  ...
  {
    name: "ESG-Auditor",
    desc: "AI-powered ESG compliance auditing platform using LangChain, Ollama, and local LLMs to analyze sustainability reports and generate audit scores.",
    stars: 0,
    forks: 0,
    lang: "Python",
    url: "https://github.com/ArnabOPP"
  ...
  {
    name: "Attendinn-System",
    desc: "Smart attendance management platform with real-time analytics, QR-based check-in, facial recognition, and admin dashboard built for EdTech.",
    stars: 0,
    forks: 0,
    lang: "TypeScript",
    url: "https://github.com/ArnabOPP"
  ...
  {
    name: "Racing-Drone",
    desc: "Custom FPV racing drone with ESP32 telemetry, RFID hardware integration, Firebase real-time data streaming, and live sensor monitoring web dashboard.",
    stars: 0,
    forks: 0,
    lang: "C++",
    url: "https://github.com/ArnabOPP"
  ...
  {
    name: "StudyX-AI",
    desc: "AI-powered study assistant mobile app with personalized learning paths, smart flashcards, and progress analytics for students.",
    stars: 0,
    forks: 0,
    lang: "Dart",
    url: "https://github.com/ArnabOPP"
...
export const GitHubShowcase = () => {
  ...
  const [pinnedRepos, setPinnedRepos] = useState(PINNED_REPOS);
  ...
  useEffect(() => {
    if (isExpanded) {
      const interval = setInterval(() => {
        setPinnedIndex((prev) => (prev + 1) % pinnedRepos.length);
      }, 3000);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // Use the token-authenticated internal API for accurate stats
        const [statsRes, eventsRes] = await Promise.all([
          fetch('/api/github-stats'),
          fetch(`https://api.github.com/users/${GITHUB_USER}/events?per_page=15`)
        ]);
        ...
        if (stats?.pinnedRepos?.length) {
          setPinnedRepos(stats.pinnedRepos);
        }
        ...
        setData(prev => ({
          ...
          stats: {
            followers: stats?.followers ?? prev.stats.followers,
            totalCommits: stats?.totalContributions ?? prev.stats.totalCommits,
            totalRepos: prev.stats.totalRepos,
            stars: stats?.totalStars ?? prev.stats.stars
          },
        }));

        // Fetch repo count + languages separately (public API, usually within rate limit)
        ...
        if (reposRes.ok) {
          ...
          validRepos.forEach((repo: any) => {
            if (repo.language) languagesMap[repo.language] = (languagesMap[repo.language] || 0) + 1;
          });
          ...
          // Update pinned repo stars/forks from real data
          const repoMap = Object.fromEntries(validRepos.map((r: any) => [r.name.toLowerCase(), r]));
          pinnedRepos.forEach(pr => {
            const match = repoMap[pr.name.toLowerCase()];
            if (match) {
              pr.stars = match.stargazers_count;
              pr.forks = match.forks_count;
            }
          });

          setData(prev => ({
            ...prev,
            stats: { ...prev.stats, totalRepos: validRepos.length },
  ...
  const currentRepo = pinnedRepos[pinnedIndex];
                  ...
                  <div className="relative bg-[#F8F8F8] dark:bg-[#111111] rounded-[2rem] p-8 border border-border/10">
                    <div className="relative z-10 flex flex-col h-full items-center justify-center gap-8">
                      ...
                      <div className="flex flex-wrap justify-center gap-4">
                        {[
                          { id: "pull-shark", label: "Pull Shark", x: 2 },
                          { id: "pair-extraordinaire", label: "Pair Extraordinaire", x: 1 },
                          { id: "yolo", label: "YOLO", x: 1 },
                          { id: "quickdraw", label: "Quickdraw", x: 1 },
                        ].map((badge, i) => (
                          <motion.div key={i} whileHover={{ scale: 1.3, rotate: -10 }} className="relative">
                            <img src={`https://github.githubassets.com/assets/${badge.id}-default-${badge.id === 'pull-shark' ? '498c279a747d' : badge.id === 'pair-extraordinaire' ? '579438a20e01' : badge.id === 'yolo' ? 'be0bbff04951' : '39c6aec8ff89'}.png`} alt={badge.label} className="w-14 h-14" />
                            {badge.x > 1 && (
                              <span className="absolute -top-1 -right-1 bg-[#39d353] text-black text-[9px] font-black rounded-full w-4 h-4 flex items-center justify-center">×{badge.x}</span>
                            )}
                          </motion.div>
                        ))}
                      </div>
