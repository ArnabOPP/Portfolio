...
// Each word is its own component so hooks are called at the top level
function ScrollWord({
    children,
    scrollProgress,
    revealStart,
    ...
    const y = useTransform(scrollProgress, [revealStart, revealEnd], [32, 0]);
    const blurPx = useTransform(scrollProgress, [revealStart, revealEnd], [12, 0]);
...
function ScrollChar({
    ...
    const opacity = useTransform(scrollProgress, [revealStart, revealEnd], [0, 1]);
    const y = useTransform(scrollProgress, [revealStart, revealEnd], [8, 0]);
...
// Pre-render fixed word count components — hooks must not be called in loops,
// so we create a fixed upper bound and only render up to wordCount
const MAX_WORDS = 20;
const MAX_CHARS = 40;
...
function BridgeHeading({
    ...
    const wordStep = range / words.length;

    // Build per-word ranges — each word reveals over 2× its share of the range
    const wordRanges = words.map((_, i) => ({
        start: revealStart + i * wordStep,
        end: revealStart + (i + 3.0) * wordStep,
    }));

    // Render up to MAX_WORDS slots; hide extras
    const slots = Array.from({ length: MAX_WORDS }, (_, i) => ({
        word: words[i] ?? null,
        range: wordRanges[i] ?? { start: 0, end: 0 },
    }));

    return (
        <p className={cn("flex flex-wrap justify-center", className)} style={{ gap: '0.3em' }}>
            {slots.map((slot, i) =>
                slot.word !== null ? (
                    <ScrollWord
                        key={i}
                        scrollProgress={scrollProgress}
                        revealStart={slot.range.start}
                        revealEnd={Math.min(slot.range.end, revealEnd + range * 0.1)}
                    >
                        {slot.word}
                    </ScrollWord>
                ) : null
...
function BridgeSubheading({
    ...
    const range = revealEnd - revealStart;
    const charStep = range / chars.length;

    const charRanges = chars.map((_, i) => ({
        start: revealStart + i * charStep,
        end: revealStart + (i + 2) * charStep,
    }));

    const slots = Array.from({ length: MAX_CHARS }, (_, i) => ({
        char: chars[i] ?? null,
        range: charRanges[i] ?? { start: 0, end: 0 },
    }));
...
function BridgeSlide({ page, isActive, scrollProgress, index }: { page: any, isActive: boolean, scrollProgress: any, index: number }) {
    ...
    const slideInStart = base - step / 4;
    const slideInEnd = base + step / 4;

    const words = (page.heading as string)?.split(' ') ?? [];

    // Heading reveal range
    const headingRevealStart = slideInStart + (slideInEnd - slideInStart) * 0.45;
    const headingRevealEnd = slideInEnd + (slideInEnd - slideInStart) * 0.7;

    // Each word's blur extends (perWordOverlap / wordCount) past headingRevealEnd.
    // actualRevealEnd is where the very last word finishes blurring in.
    const perWordOverlap = 3.0;
    const headingRange = headingRevealEnd - headingRevealStart;
    const actualRevealEnd = headingRevealStart + ((words.length - 1 + perWordOverlap) / words.length) * headingRange;

    // Subheading: starts after heading is mostly done
    const subRevealStart = slideInEnd + (slideInEnd - slideInStart) * 0.4;
    const subRevealEnd = slideInEnd + (slideInEnd - slideInStart) * 0.9;

    const opacity = useTransform(scrollProgress, [slideInStart, slideInEnd], [0, 1]);
    const y = useTransform(scrollProgress, [slideInStart, slideInEnd], [50, 0]);
    const chars = (page.subheading as string)?.split('') ?? [];
        ...
        >
            <div className="absolute inset-0 z-50 overflow-visible pointer-events-none">
                <div className="w-full h-full overflow-visible pointer-events-auto">
                    <ImageTrail items={BRIDGE_TRAIL_IMAGES} variant={2} />
                </div>
            </div>
            <motion.div style={{ y }} className="space-y-12 max-w-[1200px] w-full px-[5%] flex flex-col items-center">

                <BridgeHeading
                    words={words}
                    scrollProgress={scrollProgress}
                    revealStart={headingRevealStart}
                    revealEnd={headingRevealEnd}
                    className="text-4xl md:text-5xl lg:text-7xl font-medium tracking-tight text-foreground dark:text-white leading-[1.3] font-sans"
                />
                ...
                <div className="flex flex-col items-center gap-6 pt-6">
                    <BridgeSubheading
                        chars={chars}
                        scrollProgress={scrollProgress}
                        revealStart={subRevealStart}
                        revealEnd={subRevealEnd}
                        className="text-[11px] font-mono font-bold tracking-[0.5em] uppercase text-foreground/30 dark:text-white/30"
                    />
                    <motion.div
                        animate={{ y: [0, 8, 0] }}
