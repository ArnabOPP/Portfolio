...
export function ProjectPageContent({ project, isLowPowerMode }: { project: Project; isLowPowerMode?: boolean }) {
    ...
    // Get other projects for "More Projects" section — deterministic order, no Math.random() on server
    const [otherProjects, setOtherProjects] = useState(() =>
        portfolioData.projects.filter(p => p.id !== project.id).slice(0, 5)
    );
    useEffect(() => {
        const others = portfolioData.projects.filter(p => p.id !== project.id);
        const shuffled = [...others].sort(() => 0.5 - Math.random());
        setOtherProjects(shuffled.slice(0, 5));
    }, [project.id]);
    ...
    return (
        <div className="min-h-screen bg-background text-foreground pb-24 pt-24 sm:pt-32">
            ...
            {/* Image Lightbox - Portal to avoid removeChild conflicts */}
